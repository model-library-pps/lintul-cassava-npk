...
class npk_stress(SimulationObject):
    ...
    **Auxillary variables**
    ...
    def __call__(self, day, drv, delt = 1):
        ...
        NMINLV = self.afgen2cols(p.NMINMAXLV, k.TSUMCROP, 1)  # g N g-1 DM
        PMINLV = self.afgen2cols(p.PMINMAXLV, k.TSUMCROP, 1)  # g N g-1 DM
        KMINLV = self.afgen2cols(p.KMINMAXLV, k.TSUMCROP, 1)  # g N g-1 DM
        NMAXLV = self.afgen2cols(p.NMINMAXLV, k.TSUMCROP, 2)  # g N g-1 DM
        PMAXLV = self.afgen2cols(p.PMINMAXLV, k.TSUMCROP, 2)  # g N g-1 DM
        KMAXLV = self.afgen2cols(p.KMINMAXLV, k.TSUMCROP, 2)  # g N g-1 DM

        # # Minimum and maximum concentrations in the stems
        NMINST = self.afgen2cols(p.NMINMAXST, k.TSUMCROP, 1)  # g N g-1 DM
        PMINST = self.afgen2cols(p.PMINMAXST, k.TSUMCROP, 1)  # g N g-1 DM
        KMINST = self.afgen2cols(p.KMINMAXST, k.TSUMCROP, 1)  # g N g-1 DM
        NMAXST = self.afgen2cols(p.NMINMAXST, k.TSUMCROP, 2)  # g N g-1 DM
        PMAXST = self.afgen2cols(p.PMINMAXST, k.TSUMCROP, 2)  # g N g-1 DM
        KMAXST = self.afgen2cols(p.KMINMAXST, k.TSUMCROP, 2)  # g N g-1 DM

        # # Minimum and maximum nutrient concentrations in the storage organs
        NMINSO = self.afgen2cols(p.NMINMAXSO, k.TSUMCROP, 1)  # g N g-1 DM
        PMINSO = self.afgen2cols(p.PMINMAXSO, k.TSUMCROP, 1)  # g N g-1 DM
        KMINSO = self.afgen2cols(p.KMINMAXSO, k.TSUMCROP, 1)  # g N g-1 DM
        NMAXSO = self.afgen2cols(p.NMINMAXSO, k.TSUMCROP, 2)  # g N g-1 DM
        PMAXSO = self.afgen2cols(p.PMINMAXSO, k.TSUMCROP, 2)  # g N g-1 DM
        KMAXSO = self.afgen2cols(p.KMINMAXSO, k.TSUMCROP, 2)  # g N g-1 DM

        # # Minimum and maximum nutrient concentrations in the roots
        NMINRT = self.afgen2cols(p.NMINMAXRT, k.TSUMCROP, 1)  # g N g-1 DM
        PMINRT = self.afgen2cols(p.PMINMAXRT, k.TSUMCROP, 1)  # g N g-1 DM
        KMINRT = self.afgen2cols(p.KMINMAXRT, k.TSUMCROP, 1)  # g N g-1 DM
        NMAXRT = self.afgen2cols(p.NMINMAXRT, k.TSUMCROP, 2)  # g N g-1 DM
        PMAXRT = self.afgen2cols(p.PMINMAXRT, k.TSUMCROP, 2)  # g N g-1 DM
        KMAXRT = self.afgen2cols(p.KMINMAXRT, k.TSUMCROP, 2)  # g N g-1 DM
    ...
    def afgen2cols(self, tbl_xyz, x, col_id):
        xs = []
        y1s = []
        y2s = []
